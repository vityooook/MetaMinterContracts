import { Address, beginCell, Cell, comment, Contract, contractAddress, ContractProvider, Sender, SendMode, storeMessageRelaxed, toNano, TupleBuilder } from '@ton/core';
...
export function nftCollectionConfigToCell(config: NftCollectionConfig): Cell {
    return beginCell()
        ...
        .storeRef(buildNftCollectionContentCell(config.contentCollection, config.contentItemForCollection))
        .storeRef(beginCell().storeBuffer(Buffer.from(config.contentItemForItem)).endCell())
...
export class NftCollection implements Contract {
    ...
    async sendChangeCommissionAndContent(provider: ContractProvider, via: Sender, opts: {
        commission: bigint;
        contentCollection: string;
        contentItemForCollection: string;
        contentItemForItem: string;
        ...
        await provider.internal(via, {
            ...
            body: beginCell()
                ...
                .storeRef(buildNftCollectionContentCell(opts.contentCollection, opts.contentItemForCollection))
                .storeRef(beginCell().storeBuffer(Buffer.from(opts.contentItemForItem)).endCell())
            .endCell(),
        });
    }
    ...
    async getCollectionData(provider: ContractProvider) {
        ...
        return {
            ...
            ownerUser: result.stack.readAddress()
        };
    }
    ...
    async getItemAddress(provider: ContractProvider, nftIndex: number) {
        ...
        return {
            nftItemAddress: result.stack.readAddress()
        };
    }
